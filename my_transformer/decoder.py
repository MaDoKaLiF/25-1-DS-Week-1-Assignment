...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, 
        ...
        x = self.residual1(
            x, 
            lambda x: self.dropout1(self.self_attn(self.norm1(x), self.norm1(x), self.norm1(x), tgt_mask))
        )
        x = self.residual2(
            x, 
            lambda x: self.dropout2(self.cross_attn(self.norm2(x), memory, memory, src_mask))
        )
        x = self.residual3(
            x, 
            lambda x: self.dropout3(self.ff(self.norm3(x)))
        )
